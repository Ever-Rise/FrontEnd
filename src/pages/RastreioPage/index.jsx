        ...
        <div className={styles.inner}>
          ...
          <div className={styles.pageHeaderRow}>
            ...
            <div className={styles.headerBtns}>
              <button className={`${styles.hbtn} ${styles.blue}`}>↓ Nota Fiscal PDF</button>
              <button className={`${styles.hbtn} ${styles.outline}`}>↗ Compartilhar Atualizações</button>
            </div>
          </div>
          ...
          <div className={styles.outerCard}>
            <div className={styles.mainGrid}>
              ...
              <div className={styles.tlCard}>
                <p className={styles.sectionLabel}>↔ Histórico de Transporte</p>
                <div className={styles.tlList}>
                  {ORDER.timeline.map((item, idx) => (
                    <div key={idx} className={styles.tlItem}>
                      <div className={`${styles.tlIcon} ${item.status === 'done' ? styles.done : styles.pending}`}>
                        {item.status === 'done' && <CheckIcon />}
                      </div>
                      <div className={styles.tlBody}>
                        <p className={styles.tlLabel}>{item.label}</p>
                        <p className={`${styles.tlTitle} ${item.status === 'pending' ? styles.muted : ''}`}>
                          {item.title.split('\n').map((line, i, arr) => (
                ...
                <div className={styles.driverRow}>
                  <div className={styles.driverLeft}>
                    <div className={styles.driverThumb}>{ORDER.driver.initials}</div>
                    <div>
                      <p className={styles.driverLbl}>{ORDER.driver.label}</p>
                      <p className={styles.driverName}>{ORDER.driver.name}</p>
                    </div>
                  </div>
                ...
                <div className={styles.infoRow}>
                  <div className={styles.infoCard}>
                    <div className={styles.infoHdr}>
                      ...
                      <p className={styles.infoHdrLabel}>Endereço de Entrega</p>
                    </div>
                    <p className={styles.addrLine}>{ORDER.address.line1}</p>
                    <p className={styles.addrLine}>{ORDER.address.line2}</p>
                  </div>

                  <div className={styles.infoCard}>
                    <div className={styles.infoHdr}>
                      <div className={styles.infoIco}>💳</div>
                      <p className={styles.infoHdrLabel}>Método de Pagamento</p>
                    </div>
                    <div className={styles.payMethod}>
                      <div className={styles.payIco}>🏦</div>
                      <div>
                        <p className={styles.payName}>{ORDER.payment.name}</p>
                        <p className={styles.paySub}>{ORDER.payment.sub}</p>
                      </div>
                    </div>
                    <div className={styles.payTotalRow}>
                      <span className={styles.payTotalLbl}>Valor Total:</span>
                      <span className={styles.payTotalVal}>{ORDER.payment.total}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <tbody>
                {ORDER.items.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td><span className={styles.loteBadge}>{item.lote}</span></td>
                    <td>{item.qty}</td>
                    <td>{item.price}</td>
                  </tr>
                ))}
